    ...
    <section className="field-panel field-weather" aria-labelledby="field-weather-heading">
      ...
      <div className="field-weather-grid">{days.map(day => {
        ...
        return <article key={day.date} data-weather-day={day.date} className={`field-day field-day-${held ? 'hold' : guidance.level}`}>
          ...
          <div className="field-temperature">{Number.isFinite(day.high_f) ? `${Math.round(day.high_f)}°` : '—'}<small>{Number.isFinite(day.low_f) ? ` / ${Math.round(day.low_f)}°` : 'No forecast'}</small></div>
          <span className={`field-condition field-condition-${held ? 'hold' : guidance.level}`}>{held ? 'Crew day off' : guidance.label}</span>
          <div className="field-weather-details"><span>{Number.isFinite(day.rain_chance) ? `${Math.round(day.rain_chance)}% rain` : 'Rain unknown'}</span><span>{day.wind_coverage === 'complete' && Number.isFinite(day.wind_mph) ? `Wind up to ${Math.round(day.wind_mph)} mph` : 'Wind data incomplete'}</span></div>
      ...
      {fresh ? <div className="field-weather-source"><WeatherAttribution forecast={forecast}/><p>ABQ ADU trade guidance derived from Apple Weather. Check site conditions before starting. <a href="https://www.weather.gov/safety/lightning-job" target="_blank" rel="noreferrer">Lightning safety</a></p><small>Updated {new Date(forecast.checked_at).toLocaleString('en-US', {timeZone:'America/Denver'})} MT</small></div>
        : <p className="field-empty-note">{forecast ? 'This forecast no longer covers the next four days. Refresh before making a weather decision.' : 'Connect Apple Weather in office settings, then update the forecast. Blank cards mean weather is unknown.'}</p>}
